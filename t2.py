    ...
    for genre in genres:
        ...
        for index, movie_genres in movies['genres'].iteritems():
            if count >= num_columns:
                break  # Break loop if we already have enough recommendations

            if genre in movie_genres and index not in selected_movies:
                selected_movies.add(index)  # Add the selected movie index to the set
                # st.text(selected_movies)
                with columns[count]:
